...
tf.app.flags.DEFINE_string(
    ...
    """imagenet_2012_challenge_label_map_proto.pbtxt.""")
tf.app.flags.DEFINE_string('minmax', '',
                           """minmax""")
...
def create_graph(self):
  ...
  with tf.gfile.FastGFile(os.path.join(
    #  FLAGS.model_dir, 'classify_image_graph_def.pb'), 'rb') as f:
      FLAGS.model_dir, 'frozen_resnet_v2_50.pb'), 'rb') as f:
...
class ResnetModelPrediction:
  def __init__(self, sess, use_log = True):
    ...
    if self.use_log:
      ...
      output_name = 'resnet_v2_50/predictions/Reshape:0'
...
class ResnetModel50:
  ...
  def __init__(self, sess, use_log = False):
    ...
    if not CREATED_GRAPH:
      create_graph(self)
      CREATED_GRAPH = True
    self.model = ResnetModelPrediction(sess, use_log)
  ...
  def predict(self, img):
    ...
    if img.shape.as_list()[0]:
      ...
      softmax_tensor = tf.import_graph_def(
        ...
        input_map={'input:0': img, 'resnet_v2_50/predictions/Shape:0': shape},
        return_elements=[output_name])
    else:
      # placeholder shape
      softmax_tensor = tf.import_graph_def(
...
def main(_):
  ...
  with tf.Session() as sess:
    ...
    dat -= 0.5
    # print(dat)
    model = ResnetModelPrediction(sess, True)
...
class ImageNet:
  def __init__(self, seed):
    ...
    #file_list = sorted(os.listdir("/home/xukaidi/Workspace/ADMM/ADMM_secure_v11/specialimgs2/"))
    #random.seed(seed)
